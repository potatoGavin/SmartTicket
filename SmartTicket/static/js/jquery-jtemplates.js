            ...
            try {
                ...
                if ((e > s && step > 0) || (e < s && step < 0)) {
                    ...
                    for (; ((step > 0) ? (s < e) : (s > e)) ; s += step, ++iteration, ++loopCounter) {
                        ...
                        //
                        //2013/10/28
                        //Neale.Xu add fullindex param
                        param.pageSize = param.pageSize || 10;
                        param.currentPage = param.currentPage || 1;
                        extData[this._name + '$fullindex'] = s + (param.pageSize * (param.currentPage - 1));
        ...
        jQuery.fn.processTemplate = function (d, param, options) {
            return jQuery(this).each(function () {
                ...
                if (t != null) {
                    ...
                    //2015/01/20
                    //Neale.Xu add param.rowColor param
                    if (param  && param.rowColor) {
                        var _table = jQuery(this);
                        _table.find('tr:odd td').css('background', '#eef0f6');
                        _table.find('tr:even td').css('background', '#ffffff');
                        _table.find('tr').bind('mouseover', function () {
                            $(this).find('td').css('background', '#faf1cc');
                        });
                        jQuery(this).find('tr').bind('mouseout', function () {
                            _table.find('tr:odd td').css('background', '#eef0f6');
                            _table.find('tr:even td').css('background', '#ffffff');
                        });
                    }
